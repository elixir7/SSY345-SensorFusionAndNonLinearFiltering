function [mu_y, Sigma_y, y_s] = approxGaussianTransform(mu_x, Sigma_x, f, N)
    ...
    % 1. Draw N samples from the gaussian density, given input and call them x_s
    x_s = mvnrnd(mu_x, Sigma_x, N);

    % 2. For each vector in x_s, apply y=f(x). Concat the result y into y_s
    for i = 1:N
       y_s(i,:) = f(x_s(i,:)'); 
    end

    % 3. Calculate mean and covar of y_s'
    mu_y = mean(y_s)';
    Sigma_y = cov(y_s);
    y_s = y_s';
